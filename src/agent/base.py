...
class TradingAgent:
    ...
    def _display_fundamental_results(self, results: Dict[str, Any]):
        ...
        self.communicator.say(f"- Проанализировано новостей: {results['total_articles']}")
        ...
        if sentiment_analysis and sentiment_analysis != 'no_data':
            ...
            self.communicator.say(f"- Положительных новостей: {sentiment_analysis['positive_count']} ({sentiment_analysis['positive_ratio']:.1%})")
            self.communicator.say(f"- Отрицательных новостей: {sentiment_analysis['negative_count']} ({sentiment_analysis['negative_ratio']:.1%})")
            self.communicator.say(f"- Общая тональность: {sentiment_analysis['overall_sentiment']}")
        
        # Важные новости с их тональностью
        if results['important_news']:
            self.communicator.say("\nВажные новости и их тональность:")
            for news in results['important_news'][:5]:
                # Находим тональность для этой новости
                sentiment = "нейтральная"
                for s in sentiment_analysis.get('sentiments', []):
                    if s['title'] == news['title']:
                        sentiment = "положительная" if s['label'] == 'POSITIVE' else "отрицательная"
                        break
                        
                self.communicator.say(f"- {news['title']} ({news['source']}) - {sentiment}")
